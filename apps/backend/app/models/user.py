...
from sqlalchemy import Uuid as SAUuid   # SQLAlchemy 2.0 native cross-DB UUID type
...
class User(Base):
    ...
    # Primary Key — SAUuid works with both PostgreSQL (native UUID) and SQLite (CHAR/VARCHAR)
    ...
    race = Column(                                                           # 6. Race
        SAEnum('Malay', 'Chinese', 'Indian', 'Eurasian', 'Kadazan', 'Iban', 'Other', name='race_enum'),
        nullable=True
    )
    marital_status = Column(                                                 # 7. Marital Status
        SAEnum('single', 'married', 'divorced', 'widowed', name='marital_status_enum'),
        nullable=True
    )
    num_dependents = Column(Integer, nullable=True, server_default='0')      # 8. Number of Dependents

    # Address information (3 address fields)
    taman_name = Column(String(255), nullable=True, index=True)              # 9. Taman Name
    ...
    verification_status = Column(
        SAEnum('not_started', 'pending', 'verified', 'rejected', name='verification_status_enum'),
        nullable=False, server_default='not_started', index=True
    )
    ...
    # DateTime(timezone=True) → TIMESTAMPTZ on PostgreSQL, stored with tz info on SQLite
    ...
    # Timestamps — server-set; func.now() is DB-level default
...
class OAuthCredential(Base):
    ...
    user_id = Column(SAUuid(as_uuid=True), ForeignKey('users.id', ondelete='CASCADE'), nullable=False, index=True)
    provider = Column(
        SAEnum('google', 'microsoft', 'apple', 'github', name='oauth_provider_enum'),
...
class AuditLog(Base):
    ...
    id = Column(SAUuid(as_uuid=True), primary_key=True, default=uuid.uuid4, nullable=False)
    user_id = Column(SAUuid(as_uuid=True), ForeignKey('users.id', ondelete='SET NULL'), nullable=True, index=True)
    ...
    resource_id = Column(SAUuid(as_uuid=True), nullable=True, index=True)
